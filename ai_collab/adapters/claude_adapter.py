"""Claude-oriented push adapter with auditable fallback semantics."""
...
DEFAULT_REPORT_PATH = "logs/claude_adapter_report.json"
DEFAULT_HISTORY_PATH = "logs/claude_adapter_history.jsonl"
DEFAULT_SUMMARY_PATH = "collaboration/monitoring/CLAUDE_ADAPTER_SUMMARY_latest.md"
DEFAULT_EVENT_DIR = "collaboration/monitoring/claude_push_events"
...
def _load_claude_adapter_config(workspace: Path) -> dict[str, Any]:
    ...
    if not isinstance(session_config, dict):
        return {}
    raw = session_config.get("claudeAdapter")
...
def _resolve_paths(
    *,
    ...
    config = _load_claude_adapter_config(workspace)
...
def _adapter_mode(workspace: Path) -> tuple[bool, str, str, str]:
    config = _load_claude_adapter_config(workspace)
    ...
    return enabled, bridge_command, channel, delivery_status
...
def build_claude_push_event_markdown(
    *,
...
def _event_file(event_dir: Path, intervention_id: str) -> Path:
    return event_dir / f"CLAUDE_PUSH_EVENT_{intervention_id}_latest.md"
...
def run_claude_push_adapter(
    *,
    ...
    enabled, bridge_command, channel, delivery_status_on_success = _adapter_mode(workspace)
    payload = inspect_interventions(workspace=workspace, assignee="claude_code", only_open=True)
    ...
    for intervention in candidates:
        ...
        event_file.write_text(
            build_claude_push_event_markdown(
...
def heartbeat_claude_adapter(*, workspace: Path) -> dict[str, Any]:
    report_file, history_file, summary_file, event_root = _resolve_paths(workspace=workspace)
    enabled, bridge_command, _, _ = _adapter_mode(workspace)
    queue = inspect_interventions(workspace=workspace, assignee="claude_code", only_open=True)
    registry = inspect_sessions(workspace=workspace, assignee="claude_code")
...
def get_claude_adapter_contract() -> SessionAdapterContract:
    return SessionAdapterContract(
        name="claude",
        assignee="claude_code",
        capability="push",
        register_session=register_claude_session,
        push_interventions=run_claude_push_adapter,
        ack_delivery=ack_claude_delivery,
        heartbeat=heartbeat_claude_adapter,
    )
