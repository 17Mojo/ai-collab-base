...
# Hermes 适配: 使用 hermes 自带 logger
from hermes.logger import log

# 模拟 IntegrationMode 枚举(Hermes 内部使用)
class IntegrationMode:
    MOCK = "mock"
    FALLBACK = "fallback"
    REAL = "real"

def get_mode(name: str) -> IntegrationMode:
    """Hermes 模拟: 始终返回 FALLBACK 模式"""
    return IntegrationMode.FALLBACK
...
class ConsensusEngine:
    ...
    def __init__(self):
        # 可插拔 provider 配置
        self.models: dict[str, AIProvider] = {}  # Hermes 适配: providers → models
        # 兼容旧名(可平滑迁移)
