                        ...
                        "branches": [
                            {
                                ...
                                "regex_config": {
                                    "pattern": b.regex_config.pattern,
                                    "flags": b.regex_config.flags,
                                    "extract_fields": b.regex_config.extract_fields,
                                } if b.regex_config else None,
                                "negate": b.negate,
                                "threshold_value": b.threshold_value,
                            }
                            for b in (step.branches or [])
                        ] if step.branches else None,
        ...
        steps = [
            WorkflowStep(
                ...
                branches=[
                    BranchCondition(
                        target_step=b["target_step"],
                        condition_type=b.get("condition_type", "regex_match"),
                        target_field=b.get("target_field", "output"),
                        condition_value=b.get("condition_value", ""),
                        regex_config=RegexPattern(
                            pattern=b["regex_config"]["pattern"],
                            flags=b["regex_config"].get("flags", ""),
                            extract_fields=b["regex_config"].get("extract_fields"),
                        ) if b.get("regex_config") else None,
                        negate=b.get("negate", False),
                        threshold_value=b.get("threshold_value"),
                    )
                    for b in step.get("branches", [])
                ] if step.get("branches") else None,
