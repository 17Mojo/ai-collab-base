...
class PackExecutorMVP:
    ...
    def __init__(self, pack_data: Dict[str, Any]):
        ...
        self._workflow_raw = pack_data.get("workflow", {})
        self.context: Dict[str, Any] = {}  # 执行上下文
        self.results: List[Dict[str, Any]] = []  # 执行结果

    @property
    def workflow(self) -> Dict[str, Any]:
        """获取 workflow，兼容 list 和 dict 格式."""
        # 如果 workflow 是列表，包装成 {"steps": [...]} 格式
        if isinstance(self._workflow_raw, list):
            return {"steps": self._workflow_raw}
        return self._workflow_raw
